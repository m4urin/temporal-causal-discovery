...
class NAVAR(nn.Module):
    def __init__(self, num_nodes, num_hidden, maxlags, hidden_layers=1, dropout=0):
        ...
        self.first_hidden_layer = nn.Conv1d(num_nodes, num_hidden * num_nodes, kernel_size=maxlags,
                                            groups=num_nodes, bias=False)
        self.dropout = nn.Dropout(p=dropout)
        self.hidden_layer_list = nn.ModuleList()
        self.dropout_list = nn.ModuleList()
        for k in range(hidden_layers - 1):
            self.hidden_layer_list.append(
                nn.Conv1d(num_nodes, num_hidden * num_nodes, kernel_size=num_hidden, groups=num_nodes, bias=False))
            self.dropout_list.append(nn.Dropout(p=dropout))
        self.contributions = nn.Conv1d(num_nodes, num_nodes * num_nodes, kernel_size=num_hidden, groups=num_nodes, bias=False)
        self.biases = nn.Parameter(torch.ones(1, num_nodes) * 0.0001)

    def forward(self, x):
        print('x', x.size(), x.is_contiguous(), '\n', x)
        hidden = self.first_hidden_layer(x)
        print('hidden_conv', hidden.size(), hidden.is_contiguous(), '\n', hidden)
        hidden = hidden.clamp(min=0).transpose(-1, -2).view([-1, self.num_nodes, self.num_hidden])
        print('hidden', hidden.size(), hidden.is_contiguous(), '\n', hidden)
        hidden = self.dropout(hidden)
        for i in range(len(self.hidden_layer_list)):
            hidden = self.hidden_layer_list[i](hidden)
            print('hidden_conv', hidden.size())
            hidden = hidden.clamp(min=0).view([-1, self.num_nodes, self.num_hidden])
            print('hidden', hidden.size())
            hidden = self.dropout_list[i](hidden)

        contributions = self.contributions(hidden)
        print('contributions_conv', contributions.size(), '\n', contributions)
...
num_hidden = 6
max_lags = 2
m = NAVAR(num_nodes=num_nodes, num_hidden=num_hidden, kernel_size=kernel_size)
dat = torch.rand(1, 2, 8)
dat[:, 0] = 0

#print (dat.mean(dim=2))
#print(dat.view(-1, 5, 4).mean(dim=-1).view(2, 3, 5))


pr, co = m(dat)
co = co.std(dim=0).view(2, 2)
print(co.size())
print(co)
