...
model = NAVAR_TCN_ALEATORIC(n_variables=3, hidden_dim=10, kernel_size=2, n_blocks=2, n_layers_per_block=1)
...
scale = (1 - torch.tanh(3 * data[0, 0, :-1]) ** 2) + 0.2
...
# Function to compute KL divergence between predicted and actual distribution
def kl_divergence(_mu, _log_var):
    # Assuming target distribution has mean=0 and std=1
    kl = -0.5 * (1 + _log_var - _log_var.exp())  # - _mu.pow(2)
    return kl.mean()
